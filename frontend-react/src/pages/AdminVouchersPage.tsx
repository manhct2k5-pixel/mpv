...
import { voucherApi } from '../services/api.ts';
...
const AdminVouchersPage = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: VoucherUpsertPayload }) =>
      voucherApi.adminUpdate(id, payload),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: voucherApi.adminDelete,
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-rose-200/70 bg-white/90 p-5 shadow-[0_12px_30px_rgba(148,163,184,0.10)]">
        <h2 className="mb-4 text-base font-semibold text-cocoa">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-cocoa/70">Giá trị đơn tối thiểu (đ)</label>
            <input
              type="number"
              min={0}
              step={1000}
      ...
      <div className="rounded-2xl border border-rose-200/70 bg-white/90 p-5 shadow-[0_12px_30px_rgba(148,163,184,0.10)]">
        <h2 className="mb-4 text-base font-semibold text-cocoa">Danh sách voucher</h2>

        {isLoading ? (
          <p className="text-sm text-cocoa/50">Đang tải...</p>
        ) : vouchers.length === 0 ? (
          <p className="text-sm text-cocoa/50">Chưa có voucher nào.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-rose-100 text-xs font-semibold uppercase tracking-wide text-cocoa/60">
                  ...
                  <th className="pb-2 text-left">Tạo bởi</th>
              ...
              <tbody className="divide-y divide-rose-50">
                {vouchers.map((v) => (
                  <tr key={v.id} className="hover:bg-rose-50/40">
                    ...
                    <td className="py-2.5 text-cocoa/70">{v.createdBy}</td>
