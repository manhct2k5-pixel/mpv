...
const CheckoutPage = () => {
  ...
  const [saveForNextPurchase, setSaveForNextPurchase] = useState(true);
  ...
  useEffect(() => {
    ...
    if (isCustomer && !addressesFetched) {
      return;
    }
  ...
  }, [addresses, addressesFetched, hasHydratedAddress, isCustomer, profile]);
  ...
  const orderMutation = useMutation({
    mutationFn: () => storeApi.createOrder(formData),
    onSuccess: async (order) => {
      queryClient.invalidateQueries({ queryKey: ['store-cart'] });
      if (saveForNextPurchase) {
        try {
          await financeApi.saveDefaultAddress({
  ...
  if (!isCustomer) {
    return (
      <div className="rounded-3xl border border-rose-200/70 bg-white/90 p-6 text-sm text-cocoa/70 shadow-[0_12px_24px_rgba(148,163,184,0.14)]">
        Checkout chỉ dành cho tài khoản khách hàng (Customer).
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-8">
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1.1fr,0.9fr]">
        <div className="space-y-4">
          ...
          <div className="space-y-3 rounded-3xl border border-rose-200/70 bg-white/90 p-5 shadow-[0_12px_24px_rgba(148,163,184,0.14)]">
              ...
              <div className="grid gap-4 rounded-2xl border border-caramel/40 bg-white/80 p-4 text-sm text-cocoa/80 md:grid-cols-[0.9fr,1.1fr]">
                ...
                <div className="space-y-2">
                  ...
                  <p className="mt-2 text-xs text-cocoa/60">
                    Sau khi chuyển khoản hoặc quét QR, shop sẽ xác nhận trạng thái thanh toán trên đơn hàng.
                  </p>
                </div>
              </div>
            )}
